...
from app.models import Agent, ChatMessage, Job, User
...
def chat_job(
    ...
    return JobChatResponse(message=chat_result["message"], phase=chat_result["phase"])
...
def hire_job(
    ...
    return JobHireResponse(job_id=job.id, status=job.status)
